...
#define AUTH_INFO	"123456789"

#define DEVID		"mqttx_123456789"
	...
	switch(type)
	{
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				if(!json)//解析失败
					UsartPrintf(USART_DEBUG, "err: Parse req_payload failed!");
				else{//解析成功
						{char* str = cJSON_Print(json);
						UsartPrintf(USART_DEBUG, "json: %s\r\n",str);
						free(str);}
					json_value = cJSON_GetObjectItem(json, "target");//获取键"target"对应的值
					//UsartPrintf(USART_DEBUG, "json_value: %s\r\n",(char*)json_value->valuestring);
					if(  ! strcmp(json_value->valuestring, "led") )//json_value 为 “led”
					{	
						json_value = cJSON_GetObjectItem(json, "value");//获取键"value"对应的值
						//UsartPrintf(USART_DEBUG, "json_value: %d\r\n",json_value->valueint);
						if( (int)json_value->valueint==1 )
							{LED_ON(1);}
						else
							{LED_OFF(1);}
					}
					else if(  ! strcmp(json_value->valuestring, "alarm") )//json_value 为 “led”
					{	
						json_value = cJSON_GetObjectItem(json, "value");//获取键"value"对应的值
						if( (int)json_value->valueint==1 )
							{BEEP_ON(PWM_compare);}
						else
							{BEEP_OFF(PWM_compare);}
					}
				}
				cJSON_Delete(json);
