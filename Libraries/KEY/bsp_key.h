...
#define KEY_COMB_ON 1 //是否使用组合键
#define G_KEY_NUM 8  //所使用的按键数量(不包括组合键，不包括组合键的锚键)
#define G_KEYCOMBANCHOR_NUM 2  //所使用的组合键锚键按键数量(锚键只在组合键中起作用)
#define G_KEYCOMB_NUM 5 //所使用的组合键组数量 (组合键一组表示一个按键)
#define G_KEYCBFLIST_NUM 9 //使用回调函数列表执行回调的按键个数(包括独立按键与组合键)
...
#define G_KEY_FILTER_TIME 1 // 按下/释放 软件消抖/滤波时间 单位/10ms
#define G_KEY_LP_FT 100 // 长按判断时间 单位/10ms
#define G_KEY_LP_RT 30 // 长按重复触发时间 单位/10ms
#define G_KEY_DOUBLEPRESS_TIME 50 // 双击间隔时间 单位/10ms
...
typedef struct 
{
    ...
    uint8_t _LPRTime;//长按重复触发周期/10ms
...
{
    /* data */
    void (*pressedFun)(void);//按下(单击) (序号0)
    void (*releaseFun)(void);//按下(单击释放) (序号1)
    void (*doubleReleaseFun)(void);//双击(双击释放) (序号2)
    void (*longPressedFun)(void);//长按 (序号3)
}KEY_CALLBACK_FUN_t;
