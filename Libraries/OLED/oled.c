...
u8 OLED_GRAM[OLED_Pixel_width][OLED_Pixel_height/8];
...
uint32_t OLED_WR_Byte(u8 dat,u8 mode)
{
	...
	uint8_t pBuff[] = {0x00|mode<<5, dat};
	for(uint8_t i=0;i<2;i++)
	{
		I2C_SendData(OLED_IICx, pBuff[i]); 
		
		I2CTimeout = I2CT_FLAG_TIMEOUT;  
		/* Test on EV8 and clear it */
		while(!I2C_CheckEvent(OLED_IICx, I2C_EVENT_MASTER_BYTE_TRANSMITTING))
		{
...
uint32_t OLED_WR_Bytes(u8 *dat,u8 mode, u16 len)
{
	/* Send STRAT condition */
	I2C_GenerateSTART(OLED_IICx, ENABLE);
	SisTic_Delay_ms(10);
	...
	/* Send the byte to be written */ 
	//send cmd mode 	
	uint8_t dc_cmd = 0x00|mode<<5;
	I2C_SendData(OLED_IICx, dc_cmd); 
	I2CTimeout = I2CT_FLAG_TIMEOUT;  
	/* Test on EV8 and clear it */
	while(!I2C_CheckEvent(OLED_IICx, I2C_EVENT_MASTER_BYTE_TRANSMITTING))
	{
...
uint32_t OLED_Refresh(void)
{
	...
	for(i=0;i<4;i++)
	{
		...
		for(n=0;n<128;n++)
		{
			I2C_SendData(OLED_IICx, OLED_GRAM[n][i]); 
...
{
	u8 i,m,n;
	i=y/8; //Memory Addressing Mode 为 page addressing mode，确定点在第几页内，8bit为一页
	m=y%8; //确定点在该页的第几行
	n=1<<m;//点在第几列
...
void OLED_Display_XxX_ASCII(uint8_t XS,uint8_t YS, uint8_t width,uint8_t height, uint8_t mode, char *fmt,...)
{
	...
	if(width==8&&height==12)//无需缩放
	{
		do
		{
			...
			while(*pStr==0x0a)//\n
			{XS = 0;YS+=12;pStr++;}
			if(XS>OLED_Pixel_width)
			{
				if(YS>OLED_Pixel_height)
					return;//行列都超范围直接退出
				else
				{
					XS = 0;
