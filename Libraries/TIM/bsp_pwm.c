...
/**
  * @brief   定时器PWM功能初始化
  * @param   
  * @param PWM_TIMx：要使用的定时器
  * @param TIM_Channel_x：使用定时器的通道
  * @param GPIO_Remapping： @ref TIMx_IO_Reamp 
  * @param reload：预装载比较值
  * @param psc：时钟分频因子；频率=72M/(psc*reload)
  * @retval  
  */
void TIMx_CHx_PWM_Init(TIM_TypeDef * PWM_TIMx, uint16_t TIM_Channel_x, 
						uint32_t GPIO_Remapping ,u16 reload, u16 psc)
{

	TIM_Cmd(PWM_TIMx,DISABLE); //关闭定时器

	TIMx_CHx_PWM_IO_Config(PWM_TIMx, TIM_Channel_x,  GPIO_Remapping);

	TIMx_CHx_PWM_TIMEBASE_Config(PWM_TIMx,  reload,  psc);

	TIMx_CHx_PWM_OCx_Config(PWM_TIMx,  TIM_Channel_x);
	...
	//TIM_CCxCmd(PWM_TIMx,  TIM_Channel_x,  TIM_CCx_Enable);//开启输出	
}
...
void TIMx_CHx_PWM_IO_Config(TIM_TypeDef * PWM_TIMx, uint16_t TIM_Channel_x, uint32_t GPIO_Remapping)
{
	...
	if(GPIO_RemapFlag!=0)//某些引脚重映射需要禁用JTAG和SWD
	{
...
void TIMx_CHx_PWM_TIMEBASE_Config(TIM_TypeDef * PWM_TIMx, u16 reload, u16 psc)
{
	...
	//自动重装载
	TIM_ARRPreloadConfig(PWM_TIMx,ENABLE);	//使能ARR上的TIMx自动重装载值预加装寄存器(TIM_Period有缓冲)											
...
void TIMx_CHx_PWM_OCx_Config(TIM_TypeDef * PWM_TIMx, uint16_t TIM_Channel_x)
{
	...
	TIMx_OCxPreloadConfig(PWM_TIMx,TIM_OCPreload_Enable);//使能CCMR1上的TIMx OCxPE(TIM_Pulse有缓冲)，\
															注意此函数无法使能TIM6，TIM7
...
void TIMx_PWM1_RloadPsc_Set(TIM_TypeDef * PWM_TIMx, uint16_t TIM_Channel_x ,u16 reload, u16 psc)
{
	...
	TIM_OCInitStruct.TIM_OCPolarity = TIM_OCPolarity_High;//输出极性,当定时器计数值小于CCR1_Val时为高电平
	TIM_OCInitStruct.TIM_OutputState = TIM_OutputState_Enable;//使能
	TIM_OCInitStruct.TIM_Pulse = 0; //设置初始PWM脉冲宽度(比较值)为0	
	TIMx_OCxInit(PWM_TIMx, &TIM_OCInitStruct);//注意此函数无法初始化TIM6，TIM7
